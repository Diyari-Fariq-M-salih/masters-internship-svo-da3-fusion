...
```
## le 26 mai 2026
- fell into test and fail feedback loop of hell, for now, will generate a clean with imu vicon room 1 trajectory, using imu should help with scale issues for now using **./scripts/run_svo_v1_01_imu.sh script**
```text
estimate / GT path ratio:    1.008089867
Sim(3) scale est -> GT:      1.002211813
ATE RMSE:                    0.050385825 m
ATE median:                  0.044736948 m
ATE p95:                     0.083101942 m
matched duration:            139.1 s
matched poses:               2783

Dataset: EuRoC V1_01_easy
Trajectory: SVO with IMU enabled
Pose output: outputs/svo_v1_01_imu/svo_pose_cam_imu_tum.txt
GT comparison: outputs/svo_v1_01_imu/svo_pose_cam_imu_vs_gt_report.json
Aligned trajectory: outputs/svo_v1_01_imu/svo_pose_cam_imu_aligned_to_gt_tum.txt
```
...
Sim(3) usage note:
```text
Sim(3) is used only for evaluation/visualization against EuRoC ground truth.
It is not used inside SVO and it does not modify the raw SVO output.

Raw SVO trajectory:
outputs/svo_v1_01_imu/svo_pose_cam_imu_tum.txt
